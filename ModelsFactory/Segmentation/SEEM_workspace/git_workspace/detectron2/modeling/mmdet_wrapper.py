...
class MMDetBackbone(Backbone):
    """
    Wrapper of mmdetection backbones to use in ..

    mmdet backbones produce list/tuple of tensors, while . backbones
    produce a dict of tensors. This class wraps the given backbone to produce
    output in .'s convention, so it can be used in place of .
    backbones.
    """
...
class MMDetDetector(nn.Module):
    ...
    Input/output formats of this class follow .'s convention, so a
    mmdetection model can be trained and evaluated in ..
    """
